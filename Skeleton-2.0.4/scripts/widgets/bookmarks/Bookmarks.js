...
function createBookmarkPopup(tabId) {
    //hide current bookmarks
    ...
    addBookmarkTab.setAttribute("id", "createBookmark" + tabId);
    addBookmarkTab.setAttribute("class", "createBookmarkInputs" );

    var bookmarkViewerElement = createBookmarkViewer(tabId);

    addInputs(addBookmarkTab, bookmarkViewerElement);
    addButtons(addBookmarkTab, tabId);

    currentTab.appendChild(addBookmarkTab);
}

/**
 * Viewer to see what you're creating, superfluous but very cool
...
function createBookmarkViewer(tab) {
    ...
    bookmarkElement.setAttribute("class", bookmarkElement.getAttribute("class") + " forshow");
    // bookmarkElement.style.margin = "0 0 0 calc(50% - 36px)";
...
/**
 * Display tooltip text on bookmark
 * @param text
 * @param div
 * @param color
 */
...
function autoFillBookmarkForm(link, logo, name) {
    ...
    try {
        ...
        if(hasValidDomain(url)){
            ...
            findPageTitle(url.href, name);
        } else{
            console.log("invalid url : " + url.href);
        }
...
function hasValidDomain(url) {
    try {
        ...
        // Check if the hostname is localhost, an IP address, or a domain name with optional subdomains
        //https://stackoverflow.com/questions/161738/what-is-the-best-regular-expression-to-check-if-a-string-is-a-valid-url
        const regex = /^(?:http(s)?:\/\/)?(?:[\w-]+\.)+[\w-]+(?:\:\d+)?(?:\/[\w-.\/?%&=:]*)?$/;
        return parsedUrl.hostname.match(regex);

    }
        catch (e) {
        // URL is not valid, return false
        return false;
    }
}

/**
 * send a request to the server (I figured out how!), and search for the page title of the inputted url
 * @param url
 * @param name
 */
function findPageTitle(url, name) {
    var xhr = new XMLHttpRequest();
    xhr.onreadystatechange = function() {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            name.value = xhr.responseText;
            name.dispatchEvent(new Event("input"));
        }
    }
    console.log(encodeURIComponent(url));
    xhr.open('GET', '/get-page-title?url=' + encodeURIComponent(url) + "&userAgent=" + encodeURIComponent(navigator.userAgent), true);
    xhr.send();
}
...
function generateAddBookmarkBookmark(tabId) {
    var bookmarksContainer = getTabById(tabId).getElementsByClassName("bookmarks container")[0];
    ...
    bookmarkContainer.setAttribute("id", "Add" + tabId);
    middleContainer.setAttribute("class", "middle container");
    topRightCornerContainer.setAttribute("class", "topRightCorner container");

    middleContainer.onmouseover = function () {
        displayTextInDiv('CREATE', this, "white")
    };
    topRightCornerContainer.onmouseover = function () {
        displayTextInDiv('MOVE', this, "white")
    };

    middleContainer.onmouseleave = function () {
        purgeTextInDiv(this)
    };
    topRightCornerContainer.onmouseleave = function () {
        purgeTextInDiv(this)
    };

    middleContainer.onclick = function () {
        createBookmarkPopup(tabId)
    };
    ...
    bookmarkName.onmouseenter = function () {
        displayTextInDiv(this.innerText, this, "white")
    };
    bookmarkName.onmouseleave = function () {
        purgeTextInDiv(this);
    };
...
function displayBookmark(bookmark) {
    ...
    middleContainer.onmouseover = function () {
        displayTextInDiv('GO', this, "black")
    };
    topLeftCornerContainer.onmouseover = function () {
        displayTextInDiv('EXPAND', this, "white")
    };
    topRightCornerContainer.onmouseover = function () {
        displayTextInDiv('MOVE', this, "white")
    };
    bottomLeftCornerContainer.onmouseover = function () {
        displayTextInDiv('EDIT', this, "white")
    };
    bottomRightCornerContainer.onmouseover = function () {
        displayTextInDiv('DELETE', this, "white")
    };

    middleContainer.onmouseleave = function () {
        purgeTextInDiv(this)
    };
    topLeftCornerContainer.onmouseleave = function () {
        purgeTextInDiv(this)
    };
    topRightCornerContainer.onmouseleave = function () {
        purgeTextInDiv(this)
    };
    bottomLeftCornerContainer.onmouseleave = function () {
        purgeTextInDiv(this);
    };
    bottomRightCornerContainer.onmouseleave = function () {
        purgeTextInDiv(this);
    };

    middleContainer.onclick = function () {
    };
    topLeftCornerContainer.onclick = function () {
        onExpandBookmarkButtonClicked(this.parentNode)
    };
    topRightCornerContainer.addEventListener('mousedown', function (evt) {
        onMoveBookmarkButtonClicked(topRightCornerContainer, evt);
    });
    topRightCornerContainer.style.cursor = "grab";
    bottomLeftCornerContainer.onclick = function () {
        editBookmark(this.parentNode);
    };
    bottomRightCornerContainer.onclick = function () {
        deleteBookmark(this);
    };
...
function editBookmark(bookmark) { //the great scuffening
    ...
    updateLinkViewer(inputs[0], viewer);

    inputs[1].value = logo.src
    updateImageViewer(inputs[1], viewer);

    inputs[2].value = name.innerText
    updateNameViewer(inputs[2], viewer);
